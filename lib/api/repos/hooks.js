...
Hooks.prototype.test = function(user, repo, hook, callback)
{
  ...
  this.api.getAttribute(url, null, arguments.callback);
}
...
{
  this.api.repos._pshRequest(user, repo, event, "subscribe", callbackUrl, callback);
}
Hooks.prototype.unsubscribe = function(user, repo, event, callbackUrl, callback)
{
  this.api.repos._pshRequest(user, repo, event, "unsubscribe", callbackUrl, callback);
}
